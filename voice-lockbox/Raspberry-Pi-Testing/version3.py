...
def register_task():
    ...
    try:
        password = r.recognize_houndify(audio, client_id=HOUNDIFY_CLIENT_ID, client_key=HOUNDIFY_CLIENT_KEY)[0]
...
def open_task():
    ...
    try:
        ...
        if data[name] == password:
            ...
            open_val = False
            screen.fill(WHITE)
            open_button = Button("PASSWORD REJECTED", 10, 90, 300, 50, RED)
            open_button.draw(screen)
            pygame.display.flip()
            time.sleep(3)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and keyboard_visible:
                if button_0.is_clicked(pos):
                    ...
                    button_0.iter()
                    current_key = 0
                    current_index = (button_0.count - 1) % 3
                ...
                if button_2.is_clicked(pos):
                    ...
                    button_2.iter()
                    current_key = 2
                    current_index = (button_2.count - 1) % 3
                    current_input = key_mappings[current_key][current_index]
                if button_3.is_clicked(pos):
                    button_save.count_reset()
                    button_3.iter()
                    current_key = 3
                    current_index = (button_3.count - 1) % 3
                    current_input = key_mappings[current_key][current_index]
                if button_4.is_clicked(pos):
                    button_save.count_reset()
                    button_4.iter()
                    current_key = 4
                    current_index = (button_4.count - 1) % 3
                    current_input = key_mappings[current_key][current_index]
                if button_5.is_clicked(pos):
                    button_save.count_reset()
                    button_5.iter()
                    current_key = 5
                    current_index = (button_5.count - 1) % 3
                    current_input = key_mappings[current_key][current_index]
                if button_6.is_clicked(pos):
                    button_save.count_reset()
                    button_6.iter()
                    current_key = 6
                    current_index = (button_6.count - 1) % 3
                    current_input = key_mappings[current_key][current_index]
                if button_7.is_clicked(pos):
                    button_save.count_reset()
                    button_7.iter()
                    current_key = 7
                    current_index = (button_7.count - 1) % 3
                    current_input = key_mappings[current_key][current_index]
                if button_8.is_clicked(pos):
                    button_save.count_reset()
                    button_8.iter()
                    current_key = 8
                    current_index = (button_8.count - 1) % 3
                    current_input = key_mappings[current_key][current_index]

                if button_save.is_clicked(pos):
                    button_0.count_reset()
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not keyboard_visible :
            if current_state == "start":
